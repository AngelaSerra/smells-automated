...
def getRandoopCall(seed, configId, config, project, clazz, id, strategy, coreIndex):
  ...
  class_path = []
  #DEFECTS4J
  if (os.path.exists(  os.path.join(CASESTUDY_DIR+''+project)+"/build/classes"  )):
    class_path.append(os.path.join(CASESTUDY_DIR+''+project)+"/build/classes")
  if (os.path.exists(  os.path.join(CASESTUDY_DIR+''+project)+"/build"  )):
    class_path.append(os.path.join(CASESTUDY_DIR+''+project)+"/build")
  if (os.path.exists(  os.path.join(CASESTUDY_DIR+''+project)+"/target/classes"  )):
    class_path.append(os.path.join(CASESTUDY_DIR+''+project)+"/target/classes")
  #FREENET
  #FREENET -- nnt
  print("Check : "+CASESTUDY_DIR+''+project)
  for root, dirnames, filenames in os.walk(CASESTUDY_DIR+''+project):
    for filename in fnmatch.filter(filenames, '*.jar'):
      if "junit" not in filename:
        if "hamcrest" not in filename:
          class_path.append(os.path.join(root, filename))
          print(filename," Aggiunto")
        else:
          print(filename," Non Aggiunto")
      else:
        print(filename," Non Aggiunto")
        pass
  ...
  result += " -Dsun.lang.ClassLoader.allowArraySyntax=true "
...
def find(pattern, path):
    print("Find check :Pattern: ", pattern,"  :path: ",path)
    result = []
